...
namespace AsmFun.CommanderX16.Video
{
    ...
    public class X16VideoLayerAccessR37 : IVideoLayerAccess, IDisposable
    {
        private const int layerSize = 7;
        ...
        public X16VideoLayerAccessR37(IX16VideoMapTileAccess mapTileAccess, VideoSettings videoSettings, IComputerMemoryAccess computerMemoryAccess,
        ...
        private VideoLayerData ReloadLayer(byte layerIndex)
        {
            ...
            // Temp fix
            props.Mode = (byte)(props.TextMode || props.TextMode256c ? 0 :
            ...
            if (!props.BitmapMode)
            {
                ...
                props.HorizontalScroll = (ushort)(layerData[3] | (layerData[4] & 0xf) << 8);
                ...
                props.VerticalScroll = (ushort)(layerData[5] | (layerData[6] & 0xf) << 8);
            }
            else
            {
            ...
            {
                props.MapWidth = (ushort)(1 << ((layerData[0] >> 4) & 3) + 5);
                props.MapHeight = (ushort)(1 << (((layerData[0] >> 6) & 3) + 5));
                // Scale the tiles or text characters according to TILEW and TILEH.
                props.TileWidth = (ushort)(1 << (((layerData[2] & 1) + 3)));
                props.TileHeight = (ushort)(1 << (((layerData[2] >> 1) & 1) + 3));
            }
            else if (props.BitmapMode)
            {
                // Bitmap mode is tiled mode with a single tile
                props.TileWidth = (ushort)(((layerData[2] & 1)>0 ? 640 : 320));
            ...
            props.BitsPerPixel = (byte)(1 << props.ColorDepth);

            props.TileSize = (uint)((props.TileWidth * props.BitsPerPixel * props.TileHeight) >> 3);
            props.PaletteOffset = (byte)(layerData[4] & 0xf);
            return props;
        }
        ...
        public void SetLayerEnable(int layerIndex, bool state)
        {
            layers[layerIndex].IsEnabled = state;
        }
        ...
        public byte[] MemoryLayerDump(int layerIndex)
        {
            var buf = new byte[layerSize];
            Marshal.Copy(layerMemory_P+ (layerSize * layerIndex), buf, 0, buf.Length);
            return buf;
        }
