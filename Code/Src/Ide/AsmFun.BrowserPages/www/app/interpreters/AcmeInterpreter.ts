...
export class AcmeInterpreter implements IInterpreter{
    ...
    public InterpretLine(context: ICodeBlockContext, line: IEditorLine, fullParse: boolean = true): ICodeBlockContext {
        ...
        // Compiler Line
        if (lineWithoutComment.indexOf('!') > -1 || lineWithoutComment.indexOf('}') > -1) {
            //if (lineWithoutComment.indexOf(":") < 0)
                return this.InterpretCompilerLine(lineWithoutComment, lineWithoutCommentNotrim, line);
        }

        // Find opcode
        this.parseOpcode(lineWithoutCommentNotrim, line);
        ...
        if (line.opcode == null || line.opcode.code === "") {
            ...
            if (plusIndex > -1 && plusIndex < lineWithoutComment.length && lineWithoutComment[plusIndex + 1] != " ") {
                return this.InterpretMacroCall(line, lineWithoutComment.substring(plusIndex));
            }
    ...
    private InterpretCompilerLine(lineWithoutComment: string, lineWithoutCommentNotrim: string, line: IEditorLine): ICodeBlockContext {
        ...
        if (codeIndex > -1) {
            // Check if test
            var startDataIndex = lineWithoutComment.indexOf('!if');
            if (startDataIndex > -1)
            ...
            // Check address
            startDataIndex = lineWithoutComment.indexOf('!addr');
            if (startDataIndex > -1)
                return line.context.CreateAddrCodeBlock(line, lineWithoutComment.substring(5, codeIndex - 1));
            // macro
            startDataIndex = lineWithoutComment.indexOf('!macro');
            if (startDataIndex > -1) {
            ...
            // zone
            startDataIndex = lineWithoutComment.indexOf('!zn');
            if (startDataIndex > -1) {
                var name = this.GetName(4, lineWithoutComment);
                return line.context.CreateZone(line, name,false);
            }
            // for
            startDataIndex = lineWithoutComment.indexOf('!for');
            if (startDataIndex > -1) {
                var parameters: string[] = this.GetParameters("!for", lineWithoutComment);
        ...
        var exclIndex = lineWithoutComment.indexOf('!');
    ...
    private ConvertToProperty(name: string, propType:string, data: string): IPropertyData {
        ...
        switch (propType) {
            ...
            case "!h":
                ...
                returnData.dataType = "!hex";
                break;
            case "!fill":
            case "!fi":
                returnData.dataType = "!fill";
                // !fill AMOUNT [, VALUE]
                try {
    ...
    private parseOpcode(lineWithoutCommentNotrim: string, line: IEditorLine) {
