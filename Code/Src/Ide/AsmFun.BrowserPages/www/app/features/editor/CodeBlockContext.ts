...
import {
    IEditorFile, IEditorLine, IEditorBundle, ICodeBlockContext, CreateNewFile, IEditorLabel, CreateNewEditorLabel, IPropertyData,
    IEditorZone,    IEditorManagerData
...
export class CodeBlockContext implements ICodeBlockContext {
    ...
    constructor(appData: IEditorManagerData,bundle:IEditorBundle,file?:IEditorFile, parent?:ICodeBlockContext)
    {
        this.editorData = appData;
    ...
    public CreateChild(file?:IEditorFile): ICodeBlockContext {
       var thiss = this;
        var child = new CodeBlockContext(this.editorData,thiss.bundle, file != null? file: thiss.file,thiss);
    ...
    public CreateZone(line: IEditorLine, name: string, isLocalZone: boolean): ICodeBlockContext {
        ...
        var zone: IEditorZone | undefined = this.editorData.zones.find(x => x.name === cleanName);
    ...
    public CreateMacro(line: IEditorLine, nameDirty: string, parameters: string[]): ICodeBlockContext {
        ...
        // check if it already exists
        var macroContext = this.editorData.macros.find(x => x.name === cleanName);
    ...
    public AddAddressSetter(line: IEditorLine, nameDirty: string, address: number) {
        ...
        // check if it already exists
        var label = this.editorData.labels.find(x => x.data.name === name);
        if (label == null) {
            // Create new label
    ...
    private parseLabel(line: IEditorLine) {
        if (line.potentialLabel == null || line.potentialLabel.length < 1) 
            return;
        var label = this.editorData.labels.find(x => x.data.name === line.potentialLabel)
        if (label == null) {
    ...
    private parseMacro(line: IEditorLine) {
        ...
        var macro = this.editorData.macros.find(x => x.name === line.potentialMacro)
        if (macro == null) {
            //line.hasError = true;
            return;
        }
