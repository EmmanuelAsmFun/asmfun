...
import { IPaletteColor } from "../palette/data/PaletteData.js";
...
export class CanvasPainter {
    ...
    public constructor(setup: ICanvasPainterSetupData, controlManager: IControlManager) {
        this.data = setup.Canvas;
        this.gridControlData = setup.Canvas.Grid;
        this.setupData = setup;
        this.selection = new GridSelection(this.data.Selection, setup.Canvas.Grid);
        this.gridControl = new GridControl(setup.Grid, setup.Canvas.Grid, controlManager, setup.ControlGroupName, this.selection);
    }
    ...
    public PaintTile(xOffset: number, yOffset: number, readOffset: number, data: number[], palette: ColorPalette) {
        if (this.paintContext == null || this.paintCanvas == null) return;
        var x = 0;
        var y = 0;
        for (var i = 0; i < 256; i++) {
